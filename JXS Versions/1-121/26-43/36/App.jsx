...
function drawLiquidLightTrapcodeSphere(ctx, width, height, time, bass, mids, highs, intensity) {
  ...
  // Marker-like silk strokes are intentionally disabled.
  // The liquid identity now comes from filled flowing sheets and soft internal glow.
  if (highs > 0.55) drawSilkVein(1.02, "255,220,135", 0.018, 0.42);
...
function drawProtectedOrbitalNodes(ctx, width, height, time, bass, mids, highs, intensity) {
  const cx = width * 0.5;
  const cy = height * 0.5;
  const base = Math.min(width, height);
  const ringRadius = base * (0.355 + bass * 0.012);
  const nodeCount = 8;
  const rotation = time * 0.00016 + mids * 0.18;

  ctx.save();
  ctx.globalCompositeOperation = "screen";
  ctx.lineCap = "round";

  // Clean cyan orbital rim, drawn after the plasma so nodes visibly sit on top.
  const arcSegments = [
    { start: -0.08, end: 0.48, alpha: 0.50, width: 3.2 },
    { start: 0.56, end: 1.02, alpha: 0.34, width: 2.1 },
    { start: 1.12, end: 1.86, alpha: 0.46, width: 3.0 },
  ];

  arcSegments.forEach((seg) => {
    ctx.beginPath();
    ctx.lineWidth = seg.width + bass * 1.5;
    ctx.shadowBlur = 28 + highs * 58;
    ctx.shadowColor = `rgba(40, 220, 255, ${0.34 + highs * 0.28})`;
    ctx.strokeStyle = `rgba(56, 220, 255, ${seg.alpha * intensity})`;
    ctx.arc(cx, cy, ringRadius, Math.PI * 2 * (seg.start + rotation * 0.08), Math.PI * 2 * (seg.end + rotation * 0.08));
    ctx.stroke();
  });

  for (let i = 0; i < nodeCount; i++) {
    const angle = rotation + (Math.PI * 2 * i) / nodeCount;
    const pulse = 0.5 + 0.5 * Math.sin(time * 0.0022 + i * 1.37 + highs * 2.5);
    const x = cx + Math.cos(angle) * ringRadius;
    const y = cy + Math.sin(angle) * ringRadius;
    const nodeRadius = 4.8 + pulse * 3.2 + highs * 3.6 + (i % 3 === 0 ? bass * 2.4 : 0);
    const hue = i % 3 === 0 ? "120,245,255" : i % 3 === 1 ? "185,105,255" : "255,120,245";

    const glow = ctx.createRadialGradient(x, y, 0, x, y, nodeRadius * 6.5);
    glow.addColorStop(0, `rgba(255,255,255,${(0.62 + highs * 0.20) * intensity})`);
    glow.addColorStop(0.18, `rgba(${hue},${(0.45 + pulse * 0.25) * intensity})`);
    glow.addColorStop(0.52, `rgba(${hue},${0.16 * intensity})`);
    glow.addColorStop(1, "rgba(0,0,0,0)");
    ctx.fillStyle = glow;
    ctx.beginPath();
    ctx.arc(x, y, nodeRadius * 6.5, 0, Math.PI * 2);
    ctx.fill();

    ctx.beginPath();
    ctx.shadowBlur = 24 + highs * 44;
    ctx.shadowColor = `rgba(${hue}, ${0.76 + highs * 0.2})`;
    ctx.fillStyle = `rgba(255,255,255,${(0.72 + pulse * 0.22) * intensity})`;
...
if (showParticles && particleStrength > 0.01) {
        drawParticles(
          ...
          mood,
          time,
          intensity * particleStrength * 0.035
        );
      }
      ...
      // Bass ripple circles removed for clean liquid-light view.
          ...
          <aside className="control-card">
            ...
            {activeTab === "particles" && (
              <HudSection title="Particles">
                ...
                <p className="note">
                  Particles are off by default so the liquid-light core stays clean. Turn them on only when you want optional outer dust.
                </p>
              </HudSection>
            )}
